...
class ConfimationDialog: ThemeDialogView {
    ...
    var onBtnCancelTapped: (() -> Void)? = nil
     var onBtnSubmitTapped: (() -> Void)? = nil
    ...
    func initialize(message:String, buttonTitle:String) {
        self.initialSetup()
        ...
        self.btnSubmit.setTitle(buttonTitle, for: .normal)
    }

    func initialSetup() {
        dialogView.clipsToBounds = true
        self.backgroundColor = .clear
        self.btnSubmit?.setRound(withBorderColor: .clear, andCornerRadious: 10.0, borderWidth: 1.0)
    ...
    func show(animated:Bool){
        ...
        if let topController = Common.appDelegate.getTopWindow() {
    ...
    @IBAction func btnSubmitTapped(_ sender: UIButton) {
        if self.onBtnSubmitTapped != nil {
            self.onBtnSubmitTapped!();
        }
    }
